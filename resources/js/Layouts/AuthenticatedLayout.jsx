...
export default function AuthenticatedLayout({ header, children }) {
  ...
  // Check if we're on the Canvas page using route().current()
  const showCreateButton =
    !route().current("canvas") && !route().current("payment.checkout");
      ...
      <nav className="fixed top-0 left-0 right-0 bg-gradient-to-r from-sky-200/50 to-orange-200/50 backdrop-blur-md z-50 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            {/* Desktop Navigation - Moved button here */}
            <div className="hidden md:flex items-center space-x-4">
              {/* Create New Ticket Button - only shown when not on Canvas page */}
              {showCreateButton && (
                <Link
                  href={route("canvas")}
                  className="group relative inline-flex items-center justify-center rounded-lg bg-sky-900 px-4 py-2 text-white shadow-md transition-all duration-200 hover:bg-sky-800 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2"
                >
                  <div className="relative mr-2 h-5 w-5">
                    <Sparkles className="absolute inset-0 h-5 w-5 sparkleIcon" />
                  </div>
          ...
          <div className={`md:hidden ${isOpen ? "block" : "hidden"}`}>
            <div className="px-2 pt-2 pb-3 space-y-1 bg-white/10 backdrop-blur-md rounded-lg mt-2">
              {/* Add Create New Ticket button to mobile menu - only when not on Canvas page */}
              {showCreateButton && (
                <Link
                  href={route("canvas")}
                  className="group flex items-center justify-center gap-2 w-full px-4 py-2 rounded-lg text-base font-medium text-white bg-sky-600 hover:bg-sky-700 transition-colors duration-200"
                >
